...
def rank(value, type, patient, database, depth):
    ...
    rank_value = "{:.5f}".format((list(data["Sample Name"]).index(patient) + 1) / len(data))
    return rank_value
...
def main(vcf, bam, bed, dp, r, two_strand, one_strand, patient, outdir, type, ref_pop, database):
    ...
    try:
        ...
        f.write("\t".join(["Sample_ID", "tissue", "Absolute_Mutation_Count", "Num_Exonic_Bases_gt_%dx_Coverage" %dp,
                           "Normalized_MuB", "average_depth", "percentage20", "TMB_level", "rank"]) + "\n")
        if bases_count == 0:
            mub = 0
        else:
            mub = "{:.4f}".format(float(count_passed_filter) * 1000000 / bases_count)
        rank_value = rank(mub, type, patient, database, average_depth)
        f.write(patient + "\t" + type + "\t" + str(count_passed_filter) + "\t" + str(bases_count) + "\t" + str(mub) + "\t" +
